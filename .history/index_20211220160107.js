...
app.get('/:movie', async(req, res) => {
    ...
    let movieList = await axios.get(`https://imdb-api.com/en/API/SearchTitle/k_1c5sb914/${movie}`);
    let imgi = [];
    let title = [];
    let id = [];
...
app.get('/:movie/:id', async(req, res) => {
    ...
    let imag = await axios.get(`https://imdb-api.com/en/API/Title/k_1c5sb914/${id}`)
    image = imag.data.image;
    let video = await axios.get(`https://imdb-api.com/en/API/YouTubeTrailer/k_1c5sb914/${id}`)
    video = video.data.videoUrl;
    if (video != null && video != "")
        video = `${video.replace('watch?v=', 'embed/')}`;
    let details = await axios.get(`https://imdb-api.com/en/API/Title/k_1c5sb914/${id}`)
